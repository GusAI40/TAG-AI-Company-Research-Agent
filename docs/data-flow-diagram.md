...
2. **Request normalisation** – The handler parses the body, checks for required secrets (`PERPLEXITY_API_KEY`, `OPENAI_API_KEY`, and `OPENAI_PROJECT_ID` for `sk-proj-` keys), and forwards the cleaned payload into the orchestrator.
...
4. **Agent reasoning** – If OpenAI credentials are present, the orchestrator assembles a structured prompt that includes the Perplexity answer and sources, then invokes the in-repo OpenAI Agents workflow to generate marketing-ready company profiles with ReAct traces.
