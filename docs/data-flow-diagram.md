...
graph TD
    ...
    C -->|Gemini deep research prompt| F[Gemini API<br/>Deep Research]
    F -->|Summary + verified links| C
    C -->|Craft agent prompt| E[OpenAI/Gemini Agents<br/>(Runner workflow)]
...
4. **Gemini deep research** – When a Google AI Studio key is present, the orchestrator submits the query, Perplexity synthesis, and focus hints to Gemini Deep Research, parsing the returned JSON into a summary, KPI insights, and clickable source links.
5. **Agent reasoning** – If an agent provider passes validation (OpenAI with a usable project ID or Gemini), the orchestrator assembles a structured prompt that now includes both the Perplexity answer and Gemini deep-research payload before invoking the in-repo agent workflow to generate diligence-ready profiles with ReAct traces.
6. **Response assembly** – The runnable sequence packages the Perplexity answer, deep-research payload, agent traces, and summary profile into a single JSON response that the Vercel function returns to the browser. The UI renders the summary, citation list, and reasoning trace for the user.
