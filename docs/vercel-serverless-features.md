...
- **Purpose**: Flags outlier KPI movements using a configurable z-score sensitivity threshold.
- **Usage**: Powers the Anomaly Radar card so reviewers can spot unexpected inflections before the Q&A gauntlet.
- **Notes**: Accepts labelled metric series with optional timestamps and returns means, standard deviations, and indexed anomalies.

### `/api/automation/playbook`
- **Purpose**: Generates a LangGraph-ready automation plan from finance objectives and signal severities.
- **Usage**: Fuels the Automation Playbook Lab with triggers, action owners, cadences, and follow-up guidance.
- **Notes**: Fully deterministic and credential-free—safe for workshops or hackathons without additional API keys.

### `/api/chat`
- **Purpose**: Streams OpenAI completions through the Vercel AI SDK so users can interrogate pitches in real time.
- **Usage**: Powers the Streaming Pitch Assistant panel, combining typing animations with live token updates during diligence.
- **Notes**: Requires `OPENAI_API_KEY` (and optionally `OPENAI_PROJECT_ID`) and defaults to `gpt-4o-mini`, while honouring custom model IDs in the request body.
