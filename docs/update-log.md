...
## 2025-04-10
- Updated the `/api/health` probe for Perplexity, OpenAI, and Gemini so authenticated checks now include the correct headers and flag misconfigured keys as degraded instead of passing with 401 responses.
- Added explicit guidance when the Perplexity key is missing to help operators understand why the health card reports the integration as offline.
- Implemented `/api/voice/transcribe`, a new edge function that forwards audio payloads to OpenAI’s transcription API, returns structured error details for missing credentials or upstream failures, and documented the behaviour in the environment guide.
