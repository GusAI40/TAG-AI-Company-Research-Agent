...
const ExamplePopup: React.FC<ExamplePopupProps> = ({
  ...
  return (
    <div
      ref={exampleRef}
      className={`absolute -top-16 left-1/2 -translate-x-1/2 sm:-top-14 sm:left-8 sm:translate-x-0 ${glassStyle.card} equilibrium-chip z-10
                 flex items-center space-x-2 px-4 sm:px-5 py-2.5 sm:py-3 cursor-pointer transform transition-all duration-300
    ...
    >
      ...
      <div>
        <span className="text-xs font-medium text-[#D9D9D9] transition-colors group-hover:text-white sm:text-sm">Try an example: </span>
        <span
          className={`inline-block text-sm font-bold text-[#79C1FF] transition-all group-hover:text-white
      ...
      <ArrowRight className="h-3.5 w-3.5 text-[#79C1FF] transition-all group-hover:translate-x-0.5 group-hover:text-white" />
    </div>
  );
};
