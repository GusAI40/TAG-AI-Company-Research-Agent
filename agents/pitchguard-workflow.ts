...
export const webResearchAgent = new Agent({
  ...
  instructions:
    [
      "You are a specialised market researcher for PitchGuard. Analyse the Perplexity synthesis and sources to extract factual company attributes that can power marketing collateral for finance students. Every structured field must contain a decisive value—if Perplexity provides ranges, convert them into a single best-supported figure.",
...
export const summarizeAndDisplay = new Agent({
  ...
  instructions:
    [
      "You are the creative director for the Ole Miss Finance Club marketing studio. Compose a concise but vivid profile using the structured evidence provided so far. Build on prior reasoning rather than repeating research queries.",
      reactInstructionSuffix,
      'Craft the description for a student audience, emphasising differentiation, opportunity, and an Apple-calibre reveal that feels like a $100M product launch.',
    ].join(' '),
  model: "gpt-5",
