...
const determineProvider = (): AgentProvider => {
  ...
  throw new Error(
    'No agent provider configured. Set OPENAI_API_KEY (for OpenAI) or GEMINI_API_KEY (for Gemini), or specify PITCHGUARD_AGENT_PROVIDER.'
  );
};
