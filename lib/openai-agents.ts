...
const callGemini = async <Schema extends ZodTypeAny>(
  ...
  const rawText = textFragments.join('\n').trim();
...
export class Runner {
  ...
  async run<Schema extends ZodTypeAny>(agent: Agent<Schema>, conversation: AgentInputItem[]): Promise<RunnerResult<Schema>> {
    ...
    const candidateTexts = buildCandidateJsonStrings(rawText);

    let parsed: Schema['_type'] | undefined;
    let sanitised = rawText.trim();
    let lastError: unknown;
    for (const candidate of candidateTexts) {
      try {
        const jsonValue = JSON.parse(candidate);
        parsed = agent.outputType.parse(jsonValue);
        sanitised = JSON.stringify(jsonValue, null, 2);
        break;
      } catch (error) {
        lastError = error;
      }
    ...
    if (!parsed) {
      const message =
        lastError instanceof Error ? lastError.message : `Unable to parse JSON output: ${String(lastError)}`;
      throw new Error(`Failed to parse agent output: ${message}`);
    }
...
export function buildCandidateJsonStrings(rawText: string): string[] {
  ...
  const balanced = findBalancedJsonSubstring(trimmed);
  if (balanced) {
  ...
  for (const candidate of candidates.values()) {
    ...
    expanded.add(trimmedCandidate);

    const lastBrace = Math.max(trimmedCandidate.lastIndexOf('}'), trimmedCandidate.lastIndexOf(']'));
    if (lastBrace >= 0 && lastBrace < trimmedCandidate.length - 1) {
      const truncated = trimmedCandidate.slice(0, lastBrace + 1).trim();
      if (truncated) {
...
function findBalancedJsonSubstring(source: string): string | null {
  let inString = false;
  let escaped = false;
  let depth = 0;
  let startIndex = -1;
