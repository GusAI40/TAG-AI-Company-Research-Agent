...
const executeGeminiDeepResearch = async (
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Gemini Deep Research request failed with status ${response.status}: ${preview.slice(0, 400)}`,
    );
  }
  ...
  if (!rawText) {
    throw new Error('Gemini Deep Research response did not contain any text.');
  }
